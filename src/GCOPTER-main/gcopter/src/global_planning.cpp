...
struct Config
{
    ...
    double dilateRadius2d,dilateRadius_zupper,dilateRadius_zlower;
    double voxelWidth;
    ...
    Config(const ros::NodeHandle &nh_priv)
    {
        ...
        nh_priv.getParam("DilateRadius2d", dilateRadius2d);
        nh_priv.getParam("DilateRadius_zupper", dilateRadius_zupper);
        nh_priv.getParam("DilateRadius_zlower", dilateRadius_zlower);
...
public:
    ...
    inline void mapCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg)
    {
        if (!mapInitialized)
        {
            ...
            voxelMap.dilate(std::ceil(config.dilateRadius2d / voxelMap.getScale()),std::ceil(config.dilateRadius_zupper / voxelMap.getScale()),std::ceil(config.dilateRadius_zlower / voxelMap.getScale()));
        ...
        if (mapInitialized)
        {
            if (startGoal.size() >= 2) 
            {
                startGoal.clear();
                                ...
                                //   =-0.1        0.133        msg->pose.orientation.z*(0.511+0.1-2*0.133)
                            ...
                            // std::cout<<" msg->pose "<< msg->pose<<std::endl;  //我没弄明白用msg->pose.orientation.z是要干嘛



            const Eigen::Vector3d goal(msg->pose.position.x, msg->pose.position.y,0.398);//test 5.05734 -0.509349           3.23671   -0.453285  
            std::cout<<"goal "<<goal<<std::endl;
            if (voxelMap.query(goal) == 0)
            {
                visualizer.visualizeStartGoal(goal, 0.05, startGoal.size());
            ...
            // const Eigen::Vector3d goal1(5.05734,-0.509349,0.398);
            // const Eigen::Vector3d goal2(3.23671,-0.453285,0.398);
            // startGoal.emplace_back(goal1);
            // startGoal.emplace_back(goal2);
            //plan();
        }
