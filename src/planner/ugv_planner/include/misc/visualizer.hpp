...
#include "plan_manage/trajectory.hpp"
#include "plan_manage/quickhull.hpp"
#include "plan_manage/geo_utils.hpp"
...
public:
                          ...
                          const std::vector<Eigen::Vector3d> &route)
    {
        ...
        wayPointsMarker.scale.x = 0.02;
        wayPointsMarker.scale.y = 0.02;
        wayPointsMarker.scale.z = 0.02;
        ...
        if (route.size() > 0)
        {
            ...
            for (auto it : route)
            {
                ...
                point.x = last(0);
                point.y = last(1);
                point.z = last(2)-0.1-0.248;//减去0.1是因为前面为了corridor把astar加上了0.1
               
                routeMarker.points.push_back(point);
                point.x = it(0);
                point.y = it(1);
                point.z = it(2)-0.1-0.248;
    ...
    inline void visualizePolytope(const std::vector<Eigen::MatrixX4d> &hPolys)
    {
        ...
        meshMarker.scale.x = 0.01;
        meshMarker.scale.y = 0.01;
        meshMarker.scale.z = 0.01;
                                ...
                                const double &radius)
    {
        ...
        sphereMarkers.scale.x = 0.01;
        sphereMarkers.scale.y = 0.01;
        sphereMarkers.scale.z = 0.01;
